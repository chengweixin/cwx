 ...
 * 
 * 类似斗鱼的开源验证码
 * https://github.com/iFindTA/NHGraphCoderPro
 * 
 * 极验验证码
...
public class VCodeController {
	...
	@ModelAttribute
	public void init() { 
		// 添加@ModelAttribute，会首先调用init方法，在访问@RequestMapping方法。
...
//		}
		...
		xx = width / (codeCount + 1); // 16
		// xx = width / (codeCount );
		fontHeight = height - 2; // 28
		codeY = height - 4; // 26
...
//		init();
		...
		// 创建字体，字体的大小应该根据图片的高度来定。 Font.PLAIN
		...
		gd.setColor(Color.BLACK);
		 for (int i = 0; i < 100; i++) {
//		for (int i = 0; i < 1; i++) {
			int x = random.nextInt(width); // 0 - 80
			int y = random.nextInt(height); // 0 - 30
		...
		StringBuffer vcodeBuff = new StringBuffer();
...
//				strRand = ch;
			...
			vcodeBuff.append(strRand);
		}
		// 将四位数字的验证码保存到Session中。
		HttpSession session = req.getSession();
		session.setAttribute("validateCode", vcodeBuff.toString());
//		req.setAttribute("validateCode", vcodeBuff.toString());
		
//		req.getServletContext().setAttribute("validateCode", vcodeBuff.toString());
